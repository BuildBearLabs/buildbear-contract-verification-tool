...
// SendRawContractsToVerificationAPI sends the original contract data to the verification API
// without transforming it to the grouped format
func SendRawContractsToVerificationAPI(data map[string][]contract.ContractInfo, apiURL string) error {
	jsonData, err := json.Marshal(data)
	if err != nil {
		return utils.LogErrorf("error marshaling data: %v", err)
	}
